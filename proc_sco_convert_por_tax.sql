...
|         |                   |                  |                       |
-------------------------------------------------------------------------
|1        |M.Sivaprakash      | 21-Dec-2005      |Coding and development |
|         |                   |                  |                       |
-------------------------------------------------------------------------
|2        |M.Sivaprakash      | 04-May-2007      |Documentation          |
|         |                   |                  |                       |
-------------------------------------------------------------------------

Description of the Procedure:
	
	 1. This procedure will have different tax values of each 
	    line of the purchase receipts.
	 2. Each receipt may have taxes or charges for receipt line wise 
	    - por_prod_tax_charges.
	 3. Each receipt may have taxes or charges for class wise        
	    - por_class_tax_charges.
	 4. Each receipt may have taxes or charges for receipt wise     
	    - po_receipt_tax_charges.
         5. i) This procedure will get values from por_prod_tax_charges ,
               line by line and directly applied into por_tax_charges,
               which is having different tax values for each line.
            ii) This procedure will get values from por_class_tax_charges 
                and will be apportioned and applied into por_tax_charges.
            iii) This procedure will get values from po_receipt_tax_charges
                 and will be apportioned and applied into por_tax_charges.
         6. Each receipt will have different tax values 
            in the por_tax_charges table
         7. Each receipt line tax or charges grouped by expenses
            and non expenses and applied into Chargeble and Non-Chargeble columns.
...
CREATE OR REPLACE PROCEDURE proc_sco_convert_por_tax
(
   var_bu            	VARCHAR2,
   var_porh_mode	VARCHAR2,
...
IS 
/*
   Curosr c9 will fetch sub-contract receipt total value of
   given sub-contract receip prefix, receipt number
   and product class.
*/
   CURSOR c9 (var_prod_cls VARCHAR2)
   IS
      SELECT SUM (porl_scon_lbr_unit_cost* var_ex_rate * porl_receipt_qty) sum_of_cls_porl_amt
...
/*
   Curosr c10 will fetch total receipt value of 
   given purchase receipt prefix, receipt number.
*/
   CURSOR c10
       IS
   SELECT SUM (porl_scon_lbr_unit_cost * var_ex_rate * porl_receipt_qty) sum_of_porl_amt
 ...
 */
   ...
   FOR cr1 IN (SELECT porl_bu, porl_receipt_pfx, porl_receipt_no, porl_po_pfx,porl_po_no,
                      porl_po_seq_no,porl_seq_no, porl_po_sub_seq_no, porl_scon_lbr_unit_cost,
   ...
   LOOP
     /*
        This cursor is used to navigate the product taxes by 
        giving arguments of a  specific product. 
     */
      ...
      LOOP
         INSERT INTO por_tax_charges
              ...
              VALUES (cr2.porptc_bu, cr2.porptc_receipt_pfx,
                      ...
                      cr2.porptc_cre_by, cr2.porptc_cre_date,'R',cr2.porptc_receipt_seq_no);
      END LOOP;

      /* This cursor is used to navigate the product class 
         taxes by passing parameter as product class.
       */
      ...
      LOOP
         ...
         INSERT INTO por_tax_charges
              ...
              VALUES (cr1.porl_bu, cr1.porl_receipt_pfx, cr1.porl_receipt_no,
                      ...
                      cr1.porl_po_sub_seq_no, cr3.porctc_tc_id,
      ...
      LOOP
         ...
         INSERT INTO por_tax_charges
              ...
              VALUES (cr1.porl_bu, cr1.porl_receipt_pfx, cr1.porl_receipt_no,
                      cr1.porl_po_pfx,cr1.porl_po_no, cr1.porl_po_seq_no,
                      cr1.porl_po_sub_seq_no, cr4.porectc_tc_id,
                      var_aportion_order_amt, cr1.porl_cre_by,
                      cr1.porl_cre_date,'R',cr1.porl_seq_no);
      END LOOP;
   END LOOP;
   ...
   FOR cr5 IN (SELECT porh_exchange_rate, porl_bu, porl_receipt_pfx,
                      ...
                      porl_accepted_qty, porl_rejected_qty, porl_cre_by,
   ...
   LOOP
	  ...
	  SELECT NVL (SUM (portc_tc_unit_amt), 0)
       ...
       WHERE portc_bu = cr5.porl_bu
         ...
         AND portc_receipt_seq_no = cr5.porl_seq_no
	 ...
	 AND portc_po_pfx = cr5.porl_po_pfx
         ...
         AND portc_bu = tc_bu
         AND portc_tc_id = tc_tc_id
         AND tc_charge_flag NOT IN ('D')
         AND tc_prod_chargeable NOT IN ('N');
